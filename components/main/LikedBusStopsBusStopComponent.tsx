...
type BusStopComponentProps = {
  ...
  onLongPress: () => void; // Add this prop for drag-and-drop functionality
  isActive: boolean; // Add this prop to indicate if the item is being dragged
};
...
const LikedBusStopsBusStopComponent: React.FC<BusStopComponentProps> = (props) => {
  ...
  useEffect(() => {
    let intervalId;
    let latestBusArrivalData = busArrivalData;
    ...
    const fetchAndSetBusArrivalData = async () => {
      try {
        ...
        console.error("LikedBusStopsBusStopComponent.tsx: Failed to fetch bus data:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const busesNotInOperation = props.allBusServices
    ?.filter((service) => !busArrivalData.some((bus) => bus.ServiceNo === service))
    ?.sort((a, b) => {
  ...
  return (
    <View style={[styles.outerContainer, props.isActive && styles.activeContainer]}>
      <TouchableOpacity
        onPress={() => setIsCollapsed(!isCollapsed)}
        onLongPress={props.onLongPress} // Enable drag on long press
        ...
        <View style={styles.busesContainer}>
          {isLoading ? (
            ...
            <View style={styles.noBusesWrapper}>
              <Text style={styles.noBusesText}>No buses are currently in operation</Text>
            </View>
          )}
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  activeContainer: {
    opacity: 0.5, // Style for when the item is being dragged
  },
  container: {
    flex: 1,
...
export default LikedBusStopsBusStopComponent;